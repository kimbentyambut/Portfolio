...
const ScrollArrow = ({ direction = "down", text, visible = true, onClick }) => {
  return (
    <div
      className={`fixed bottom-8 right-8 z-20 transition-all duration-1000 cursor-pointer ${
        visible
...
const myTechStack = [
  ...
  // FIXED: Updated scroll detection logic
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollY < windowHeight * 0.8) {
        setVisibleSection(0); // Home section
      } else if (scrollY < windowHeight * 1.8) {
        setVisibleSection(1); // Tech Stack section
      } else if (scrollY < windowHeight * 2.8) {
        setVisibleSection(2); // Projects section
      } else if (scrollY < windowHeight * 3.8) {
        setVisibleSection(3); // Playground section
      } else {
        setVisibleSection(4); // Contact section
      }
    };
  ...
  // FIXED: Updated arrow props logic to match section numbers
  const getArrowProps = () => {
    switch (visibleSection) {
      case 0:
        return { visible: true, text: "Scroll to see my Tech Stack", onClick: () => scrollToSection(1) };
      case 1:
        return { visible: true, text: "Explore my Projects", onClick: () => scrollToSection(2) };
      case 2:
        return { visible: true, text: "Visit My Playground", onClick: () => scrollToSection(3.2) };
      case 3:
        return { visible: true, text: "Get in Touch", onClick: () => scrollToSection(4.5) };
      case 4:
        return { visible: false }; // Hide arrow on contact section
      default:
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="relative z-10">
        ...
        {/* Section 4 - My Playground */}
        <section className="w-full min-h-screen flex flex-col items-center justify-center bg-black/30 py-16 mt-10">
         <div className="playground-grid">
  <div className="playground-item">
    <TypeRacerGame />
  </div>
  {/* Add more playground items here */}
</div>
        </section>
